...
def naive_scavenging(field):
    ...
    for perm in list_of_perms:
        total_gold = field[0][0] # how we will access the field in ed list notation    
        col = 0
        row = 0
        ...
        # after getting to the edge of th field, check if the path had more gold than prev path
        if total_gold > max_gold:
            max_gold = total_gold
            output = f"{max_gold} can be acquired by moving \t{perm}"
    return output
...
def dp_scavenging(field):
    ...
    for col in range(field_length - 1, -1, -1):
        for row in range(field_length):
            ...
            northeast = 0
            southeast = 0
            # look at gold collected if we choose east
            if col != field_length -1:
                east = gold_cache[row][col + 1]
            # look at gold collected if we choose northeast
            if row != 0 and col != field_length -1:
                northeast = gold_cache[row - 1][col + 1]
            # look at gold collected if we choose southeast
            if row != field_length - 1 and col != field_length -1:
                southeast = gold_cache[row + 1][col + 1]
                
            # update the cache with how much gold w collected from choosing the path that will lead to the most gold = the gold from the location are currently standing in
            # my reasoning for not using a max function si so that my max can keep track of the path that we follow to find the gold
    ...
    # because we are required to start in the northwest corner, the max gold collected will be the val and the path starts at [0][0] 
